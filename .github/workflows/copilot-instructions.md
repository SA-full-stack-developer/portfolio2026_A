...
- Jobs: lint → test → build-backend → deploy-backend, build-lab001 → deploy-lab001, build-frontend → deploy-frontend (todos tras `test`, en paralelo)
- Deploy solo en rama `develop`
- Node 22 en todos los jobs
- Frontend desplegado en rama `deploy`, backend en rama `deploy-api`, lab001 en rama `deploy-lab1`
