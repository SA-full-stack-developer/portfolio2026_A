...
const buildConfiguration = async (defaultConfigResolver, customConfigResolver, projectRoot, workspaceRoot, config = 'jest.config.js') => {
...
class JestConfigurationBuilder {
    ...
    async buildConfiguration(projectRoot, workspaceRoot, config = 'jest.config.js') {
        const pathToProject = (0, core_1.resolve)(workspaceRoot, projectRoot);
        return await buildConfiguration(this.defaultConfigResolver, this.customConfigResolver, pathToProject, workspaceRoot, config);
    }
}
