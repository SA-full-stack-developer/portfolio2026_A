export interface SchemaObject {
    ...
    colors?: boolean;
    /**
     * Path to jest config file, or a Jest configuration object. If not specified, Jest will
     * look for jest.config.js in the project root.
     */
    config?: {
        [key: string]: any;
    } | string;
    ...
    /**
     * Select between Babel and V8 to collect coverage.
     */
    ...
    forceExit?: boolean;
    /**
     * Enabled global mocks. Only 'matchMedia' is supported in v21+.
     */
    globalMocks?: GlobalMock[];
    /**
    ...
    haste?: string;
    /**
     * Ignore the tests of the specified projects. Jest uses the attribute `displayName` in the
     * configuration to identify each project.
     */
    ignoreProjects?: string[];
    /**
     * Should Jest inject global variables or not.
     */
    ...
    /**
     * Specifies the maximum number of tests that are allowed to run concurrently. This only
    ...
    /**
     * Print a warning about probable open handles if Jest does not exit cleanly after this
    ...
    projects?: string[];
    /**
     * Shuffle the order of the tests within a file. Use `--seed` to specify the seed value.
     */
    randomize?: boolean;
    /**
    ...
    /**
     * Sets a seed value that can be retrieved in a tests file via `jest.getSeed()`. The seed
     * value must be between `-0x80000000` and `0x7fffffff` inclusive.
     */
    seed?: number;
    /**
     * Run the tests of the specified projects. Jest uses the attribute `displayName` in the
     * configuration to identify each project.
     */
    ...
    /**
     * Regexp pattern strings matched against test paths. Pass multiple patterns separated by
     * spaces or repeat the flag.
     */
    ...
    /**
     * Allows to specify a custom test runner. The default is `jest-circus/runner`. A path to a
     * custom test runner can be provided: `<rootDir>/path/to/testRunner.js`.
     */
    testRunner?: string;
    /**
     * Allows to specify a custom test sequencer. The default is `@jest/test-sequencer`. A path
     * to a custom test sequencer can be provided: `<rootDir>/path/to/testSequencer.js`.
     */
    ...
    version?: boolean;
    /**
     * Gives one event loop turn to handle `rejectionHandled`, `uncaughtException` or
     * `unhandledRejection`.
     */
    ...
    /**
     * Use zoneless testing environment. When true (default), configures Jest for Angular 21+
     * zoneless applications. When false, configures Jest for applications that require zone.js
     * change detection.
     */
    zoneless?: boolean;
}
...
export declare enum GlobalMock {
