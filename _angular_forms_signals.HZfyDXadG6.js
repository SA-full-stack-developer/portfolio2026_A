import { a as M, b as x, d as I } from "@nf-internal/chunk-PZNONLPT";
import * as b from "@angular/core";
import { InjectionToken as Et, debounced as fr, computed as K, \u0275chain as mr, resource as gr, \u0275isPromise as yt, linkedSignal as wt, inject as N, \u0275RuntimeError as Ve, untracked as T, signal as St, CSP_NONCE as yr, input as Rt, Renderer2 as pr, DestroyRef as br, Injector as Tt, ElementRef as vr, afterRenderEffect as Nr, effect as Dt, makeEnvironmentProviders as Mr, declareExperimentalWebMcpTool as Er } from "@angular/core";
import { \u0275FORM_CONTROL_INTEGRATION as Ct, Validators as At, \u0275setNativeDomProperty as Xe, NG_VALIDATORS as wr, \u0275isNativeFormElement as Sr, \u0275isTextualFormElement as Rr, NG_VALUE_ACCESSOR as Tr, \u0275selectValueAccessor as Dr, \u0275elementAcceptsMinMax as Cr, NgControl as Ar } from "@angular/forms";
import { untracked as c, \u0275RuntimeError as E, computed as u, \u0275isInParamsFunction as Lt, \u0275setInParamsFunction as Je, runInInjectionContext as z, Injector as ot, linkedSignal as X, signal as W, APP_ID as Kt, effect as jt, InjectionToken as xt, inject as et } from "@angular/core";
import { AbstractControl as Bt, FormGroup as hn, FormArray as fn } from "@angular/forms";
import { SIGNAL as tt } from "@angular/core/primitives/signals";
var ce = 0;
...
var A = Symbol(), ie = Symbol(), U = class {
    ...
    push(e) { this.fns.push(nt(this.predicates, e)); }
    mergeIn(e) { let r = this.predicates ? e.fns.map(n => nt(this.predicates, n)) : e.fns; this.fns.push(...r); }
...
function nt(t, e) { return t.length === 0 ? e : r => { for (let n of t) {
...
var V = class {
    ...
    depth;
    logic;
    constructor(e, r, n) { this.builder = e, this.predicates = r, this.depth = n, this.logic = e ? Gt(e, r, n) : new V([]); }
    getChild(e) { let r = this.builder ? at(this.builder, e) : []; if (r.length === 0)
...
function at(t, e) { if (t instanceof L)
    return t.all.flatMap(({ builder: r, predicate: n }) => { let i = at(r, e); return n ? i.map(({ builder: s, predicates: o }) => ({ builder: s, predicates: [...o, n] })) : i; }); if (t instanceof H)
...
function Gt(t, e, r) { let n = new V(e); if (t instanceof L) {
...
var dt = Symbol("PATH"), g = class t {
    keys;
    parent;
    keyInParent;
    root;
    children = new Map;
    fieldPathProxy = new Proxy(this, $t);
    logicBuilder;
    ...
    static unwrapFieldPath(e) { return e[dt]; }
    static newRoot() { return new t([], void 0, void 0, void 0); }
}, $t = { get(t, e) { return e === dt ? t : t.getChild(e).fieldPathProxy; } }, Y, B = new Map, P = class t {
    schemaFn;
...
function qt(t) { return t instanceof P || typeof t == "function"; }
function w(t) { if (Y !== g.unwrapFieldPath(t).root)
    throw new E(1908, !1); }
function h(t, e, r) { return w(t), g.unwrapFieldPath(t).builder.addMetadataRule(e, r), e; }
var S = { list() { return { reduce: (t, e) => e === void 0 ? t : [...t, e], getInitial: () => [] }; }, min() { return { reduce: (t, e) => t === void 0 || e === void 0 ? t ?? e : e < t ? e : t, getInitial: () => { } }; }, max() { return { reduce: (t, e) => t === void 0 || e === void 0 ? t ?? e : e > t ? e : t, getInitial: () => { } }; }, or() { return { reduce: (t, e) => t || e, getInitial: () => !1 }; }, and() { return { reduce: (t, e) => t && e, getInitial: () => !0 }; }, override: zt };
function zt(t) { return { reduce: (e, r) => r, getInitial: () => t?.() }; }
...
function ut(t, e) { return new G(e ?? S.override(), t); }
function Re() { return f(); }
var Te = f(S.or()), oe = Re(), De = f(S.max()), Ce = f(S.max()), ae = Re(), Ae = f(S.min()), Ie = f(S.min()), Pe = f(S.max()), ke = f(S.min()), Oe = f(S.list());
function v(t, e) { if (t === e)
    return !0; if (!t || !e || t.length !== e.length)
    return !1; for (let r = 0; r < t.length; r++)
    if (!Object.is(t[r], e[r]))
        return !1; return !0; }
function Wt(t) { return t.errors().length > 0 ? "invalid" : t.pending() ? "unknown" : "valid"; }
var ye = class {
    node;
    ...
    syncErrors = u(() => this.shouldSkipValidation() ? [] : [...this.node.logicNode.logic.syncErrors.compute(this.node.context), ...this.syncTreeErrors(), ...Yt(this.node.submitState.submissionErrors())], { equal: v });
    syncValid = u(() => this.shouldSkipValidation() ? !0 : this.node.structure.reduceChildren(this.syncErrors().length === 0, (e, r) => r && e.validationState.syncValid(), Ht));
    ...
    errorSummary = u(() => { let e = this.node.structure.reduceChildren(this.errors(), (r, n) => [...n, ...r.errorSummary()]); return (typeof ngServerMode > "u" || !ngServerMode) && c(() => e.sort(Xt)), e; }, { equal: v });
    pending = u(() => this.node.structure.reduceChildren(this.asyncErrors().includes("pending"), (e, r) => r || e.validationState.asyncErrors().includes("pending")));
    status = u(() => { if (this.shouldSkipValidation())
        return "valid"; let e = Wt(this); return this.node.structure.reduceChildren(e, (r, n) => n === "invalid" || r.validationState.status() === "invalid" ? "invalid" : n === "unknown" || r.validationState.status() === "unknown" ? "unknown" : "valid", r => r === "invalid"); });
...
function Yt(t) { return t === void 0 ? [] : R(t) ? t : [t]; }
function k(t, e) { if (R(t))
    for (let r of t)
        r.fieldTree ??= e;
else
    t && (t.fieldTree ??= e); return t; }
function it(t) { return t.formField ? t.formField.element : t.fieldTree().formFieldBindings().reduce((e, r) => !e || !r.element ? e ?? r.element : e.compareDocumentPosition(r.element) & Node.DOCUMENT_POSITION_PRECEDING ? r.element : e, void 0); }
function Xt(t, e) { let r = it(t), n = it(e); return r === n ? 0 : r === void 0 || n === void 0 ? r === void 0 ? 1 : -1 : r.compareDocumentPosition(n) & Node.DOCUMENT_POSITION_PRECEDING ? 1 : -1; }
var te = f(), pe = class {
    node;
    cache = new WeakMap;
    ...
    resolve(e) { if (!this.cache.has(e)) {
        let r = u(() => { let n = g.unwrapFieldPath(e), i = this.node, s = Ut(); for (; s > 0 || !i.structure.logic.hasLogic(n.root.builder);)
    ...
    valueOf = e => { let r = this.resolve(e)().value(); if (r instanceof Bt)
...
}, Zt = { get(t, e, r) { let n = t(), i = n.structure.getChild(e); if (i !== void 0)
...
function Qt(t, e) { let r = u(() => t()[e()]); return r[tt] = t[tt], r.set = n => { Object.is(c(r), n) || t.update(i => Jt(i, n, e())); }, r.update = n => { r.set(n(c(r))); }, r.asReadonly = () => r, r; }
function Jt(t, e, r) { if (R(t)) {
    let n = [...t];
    return n[r] = e, n;
}
else
    return x(M({}, t), { [r]: e }); }
var F = Symbol(""), lt = u(() => !1), re = class {
    logic;
    node;
    createChildNode;
    ...
    _injector = void 0;
    _anyChildHasLogic;
    get injector() { return this._injector ??= ot.create({ providers: [], parent: this.fieldManager.injector }), this._injector; }
    ...
    createKeyOrOrphanSignals(e, r, n) { if (e === "root")
        return { keyInParent: ct, isOrphaned: lt }; let i = this.parent, s = n, o = u(() => { if (i.structure.isOrphaned())
        return F; let l = i.structure.childrenMap(); if (!l)
        return F; let m = l.byPropertyKey.get(s); if (m && m.node === this.node)
        return s; if (r === void 0)
        return F; for (let [j, le] of l.byPropertyKey)
        if (le.node === this.node)
            return s = j; return F; }), a = u(() => o() === F); return { keyInParent: u(() => { let l = o(); if (l === F)
    ...
    computeChildrenMap(e, r, n) { if (!Z(e) || !n && r === void 0 && !(this._anyChildHasLogic ??= this.logic.anyChildHasLogic()))
        return; r ??= { byPropertyKey: new Map }; let i, s = R(e); r !== void 0 && (s ? i = tr(r, e, this.identitySymbol) : i = rr(r, e)); for (let o of Object.keys(e)) {
        let a, d = e[o];
        if (d === void 0) {
    ...
    get pathKeys() { return er; }
    get keyInParent() { return ct; }
    isOrphaned = lt;
    childrenMap;
    ...
    constructor(e, r, n, i, s, o) { super(r, e, o), this.logic = r, this.parent = n, this.root = this.parent.structure.root; let a = this.createKeyOrOrphanSignals("child", i, s); this.isOrphaned = a.isOrphaned, this.keyInParent = a.keyInParent, this.pathKeys = u(() => [...n.structure.pathKeys(), this.keyInParent()]), this.value = Qt(this.parent.structure.value, this.keyInParent), this.childrenMap = this.createChildrenMap(), this.fieldManager.structures.add(this); }
};
var er = u(() => []), ct = u(() => { throw new E(1905, !1); });
function tr(t, e, r) { let n, i = new Set(t.byPropertyKey.keys()), s = t.byTrackingKey && new Set(t.byTrackingKey.keys()); for (let o = 0; o < e.length; o++) {
    let a = e[o];
...
function rr(t, e) { let r; for (let n of t.byPropertyKey.keys())
...
var Me = class {
    ...
    getBindingForFocus() { let e = this.formFieldBindings().filter(r => r.focus !== void 0).reduce(st, void 0); return e || this.structure.children().map(r => r.getBindingForFocus()).reduce(st, void 0); }
    ...
    get maxLength() { return this.metadata(ke); }
    get min() { let e = this.metadata(oe)?.(); return e ? this.metadata(e) : void 0; }
    get minLength() { return this.metadata(Pe); }
    get pattern() { return this.metadata(Oe) ?? nr; }
    get required() { return this.metadata(Te) ?? ir; }
...
function st(t, e) { return t ? e && t.element.compareDocumentPosition(e.element) & Node.DOCUMENT_POSITION_PRECEDING ? e : t : e; }
var Ee = class {
    node;
    selfTouched = W(!1);
    selfDirty = W(!1);
    ...
    formFieldBindings = W([]);
    constructor(e) { this.node = e; }
    dirty = u(() => { let e = this.selfDirty() && !this.isNonInteractive(); return this.node.structure.reduceChildren(e, (r, n) => n || r.nodeState.dirty(), rt); });
    touched = u(() => { let e = this.selfTouched() && !this.isNonInteractive(); return this.node.structure.reduceChildren(e, (r, n) => n || r.nodeState.touched(), rt); });
    ...
    rootName;
    submitOptions;
    constructor(e, r, n) { this.injector = e, this.rootName = r ?? `${this.injector.get(Kt)}.form${sr++}`, this.submitOptions = n; }
    structures = new Set;
    createFieldManagementEffect(e) { jt(() => { let r = new Set; this.markStructuresLive(e, r); for (let n of this.structures)
...
function or(t) { let e, r, n; return t.length === 3 ? [e, r, n] = t : t.length === 2 ? qt(t[1]) ? [e, r] = t : [e, n] = t : [e] = t, [e, r, n]; }
function ar(...t) { let [e, r, n] = or(t), i = n?.injector ?? et(ot), s = z(i, () => P.rootCompile(r)), o = new Se(i, n?.name, n?.submission), a = n?.adapter ?? new we, d = $.newRoot(o, e, s, a); o.createFieldManagementEffect(d.structure); let { experimentalWebMcpTool: l } = n ?? {}; if (l) {
    let m = z(i, () => et(Fe, { optional: !0 }));
    m && z(i, () => m(d.fieldTree, { name: l.name, description: l.description }));
} return d.fieldTree; }
function dr(t, e) { w(t); let r = g.unwrapFieldPath(t).getChild(A).fieldPathProxy; ht(r, e); }
function ht(t, e) { w(t), g.unwrapFieldPath(t).mergeIn(P.create(e)); }
function ft(t, e, r) { w(t), g.unwrapFieldPath(t).mergeIn(P.create(r), { fn: e, path: t }); }
function ur(t, e, r) { ft(t, ({ value: n }) => e(n()), r); }
function _e(t, e) { return I(this, null, function* () { let r = c(t); if (c(r.submitState.submitting))
    ...
    throw new E(1915, !1); r.markAsTouched(); let o = e?.onInvalid, a = cr(r, e?.ignoreValidators); try {
    if (a) {
        r.submitState.selfSubmitting.set(!0);
        let d = yield c(() => s?.(n, i));
        return d && hr(r, d), !d || R(d) && d.length === 0;
    }
    else
        c(() => o?.(n, i));
    return !1;
...
function lr(t) { return P.create(t); }
function cr(t, e) { switch (e) {
    case "all": return !0;
    case "none": return c(t.valid);
    default: return !c(t.invalid);
} }
function hr(t, e) { R(e) || (e = [e]); let r = new Map; for (let n of e) {
...
function mt(t) { if (t.length === 0)
    return null; let e = {}; for (let r of t)
    e[r.kind] = r instanceof ne ? r.context : r; return e; }
function gt(t, e) { return t === null ? [] : Object.entries(t).map(([r, n]) => new ne({ context: n, kind: r, control: e })); }
import { DOCUMENT as Ir } from "@angular/common";
import { httpResource as Pr } from "@angular/common/http";
import "@angular/core/primitives/signals";
var It = new Et("");
function Dn(t) { return [{ provide: It, useValue: t }]; }
function Cn(t, e) { w(t); let r = g.unwrapFieldPath(t), n; typeof e == "function" || typeof e == "string" ? n = e : n = e?.when, r.builder.addDisabledReasonRule(i => { let s = !0; return typeof n == "string" ? s = n : n && (s = n(i)), typeof s == "string" ? { fieldTree: i.fieldTree, message: s } : s ? { fieldTree: i.fieldTree } : void 0; }); }
function An(t, e) { w(t); let r = g.unwrapFieldPath(t), n = typeof e == "function" ? e : e.when; r.builder.addHiddenRule(n); }
function In(t, e) { w(t); let r = g.unwrapFieldPath(t), n; typeof e == "object" && e !== null && "when" in e ? n = e.when ?? (() => !0) : typeof e == "function" ? n = e : n = () => !0, r.builder.addReadonlyRule(n); }
function Pt(t) { let e = t; return typeof e.length == "number" ? e.length : e.size; }
function y(t, e) { return t instanceof Function ? t(e) : t; }
function q(t) { return typeof t == "number" ? isNaN(t) : t === "" || t === !1 || t == null; }
function pt(t) { return t === void 0 ? [] : Array.isArray(t) ? t : [t]; }
function D(t, e) { w(t), g.unwrapFieldPath(t).builder.addSyncErrorRule(n => k(e(n), n.fieldTree)); }
function kr(t) { return new Le(t); }
function Or(t, e) { return new Ke(t, e); }
function Fr(t, e) { return new je(t, e); }
...
var p = class {
    ...
    kind = "parse";
}, Pn = p, Br = /^(?=.{1,254}$)(?=.{1,64}@)[a-zA-Z0-9!#$%&'*+/=?^_`{|}~-]+(?:\.[a-zA-Z0-9!#$%&'*+/=?^_`{|}~-]+)*@[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?(?:\.[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?)*$/;
function kn(t, e) { D(t, r => { if (!(e?.when && !e.when(r)) && !q(r.value()) && !Br.test(r.value()))
    return e?.error ? y(e.error, r) : xr({ message: y(e?.message, r) }); }); }
function On(t, e, r) { let n = f(); h(t, n, i => { if (!(r?.when && !r.when(i)))
    return typeof e == "function" ? e(i) : e; }), h(t, Ie, ({ state: i }) => i.metadata(n)()), h(t, ae, () => Ie), D(t, i => { let s = i.value(); if (s === null || Number.isNaN(s))
    return; let o = i.state.metadata(n)(); if (!(o === void 0 || Number.isNaN(o)) && s > o)
    return r?.error ? y(r.error, i) : _r(o, { message: y(r?.message, i) }); }); }
function Fn(t, e, r) { let n = f(); h(t, n, i => { if (!(r?.when && !r.when(i)))
    return typeof e == "function" ? e(i) : e; }), h(t, Ae, ({ state: i }) => i.metadata(n)()), h(t, ae, () => Ae), D(t, i => { let s = i.value(); if (s === null || Number.isNaN(s.getTime()))
...
function _n(t, e, r) { let n = h(t, f(), i => { if (!(r?.when && !r.when(i)))
    return typeof e == "number" ? e : e(i); }); h(t, ke, ({ state: i }) => i.metadata(n)()), D(t, i => { if (q(i.value()))
    return; let s = i.state.metadata(n)(); if (s !== void 0 && Pt(i.value()) > s)
    return r?.error ? y(r.error, i) : Kr(s, { message: y(r?.message, i) }); }); }
function Vn(t, e, r) { let n = f(); h(t, n, i => { if (!(r?.when && !r.when(i)))
    return typeof e == "function" ? e(i) : e; }), h(t, Ce, ({ state: i }) => i.metadata(n)()), h(t, oe, () => Ce), D(t, i => { let s = i.value(); if (s === null || Number.isNaN(s))
    return; let o = i.state.metadata(n)(); if (!(o === void 0 || Number.isNaN(o)) && s < o)
    return r?.error ? y(r.error, i) : Or(o, { message: y(r?.message, i) }); }); }
function Ln(t, e, r) { let n = f(); h(t, n, i => { if (!(r?.when && !r.when(i)))
    return typeof e == "function" ? e(i) : e; }), h(t, De, ({ state: i }) => i.metadata(n)()), h(t, oe, () => De), D(t, i => { let s = i.value(); if (s === null || Number.isNaN(s.getTime()))
    return; let o = i.state.metadata(n)(); if (!(o === void 0 || Number.isNaN(o.getTime())) && s < o)
    return r?.error ? y(r.error, i) : Fr(o, { message: y(r?.message, i) }); }); }
function Kn(t, e, r) { let n = h(t, f(), i => { if (!(r?.when && !r.when(i)))
    return typeof e == "number" ? e : e(i); }); h(t, Pe, ({ state: i }) => i.metadata(n)()), D(t, i => { if (q(i.value()))
    return; let s = i.state.metadata(n)(); if (s !== void 0 && Pt(i.value()) < s)
    return r?.error ? y(r.error, i) : Lr(s, { message: y(r?.message, i) }); }); }
...
function xn(t, e) { let r = h(t, f(), n => e?.when ? e.when(n) : !0); h(t, Te, ({ state: n }) => n.metadata(r)()), D(t, n => { if (n.state.metadata(r)() && q(n.value()))
    return e?.error ? y(e.error, n) : kr({ message: y(e?.message, n) }); }); }
function kt(t, e) { w(t); let r = g.unwrapFieldPath(t), n = ut((i, s) => { if (e.debounce !== void 0) {
    let o = fr(() => s(), e.debounce), a = K(() => mr(o));
    return e.factory(a);
...
function Ur(t, e) { w(t), g.unwrapFieldPath(t).builder.addSyncTreeErrorRule(n => k(e(n), n.fieldTree)); }
function Bn(t, e) { let r = h(t, f(), n => { let i = typeof e == "function" ? e(n) : e; return i ? i["~standard"].validate(n.value()) : void 0; }); Ur(t, ({ state: n, fieldTreeOf: i }) => { let s = n.metadata(r)(); return !s || yt(s) ? [] : s?.issues?.map(o => bt(i(t), o)) ?? []; }), kt(t, { params: ({ state: n }) => { let i = n.metadata(r)(); return i && yt(i) ? i : void 0; }, factory: n => gr({ params: n, loader: s => I(null, [s], function* ({ params: i }) { return (yield i)?.issues ?? []; }) }), onSuccess: (n, { fieldTreeOf: i }) => n.map(s => bt(i(t), s)), onError: () => { } }); }
function Hr(t, e) { return new qe(t, e); }
function bt(t, e) { let r = t; for (let n of e.path ?? []) {
    let i = typeof n == "object" ? n.key : n;
    r = r[i];
} return k(Hr(e, { message: e.message }), r); }
var qe = class extends p {
    issue;
    kind = "standardSchema";
    constructor(e, r) { super(r), this.issue = e; }
};
function Un(t, e) { kt(t, { params: e.request, debounce: e.debounce, factory: r => Pr(r, e.options), onSuccess: e.onSuccess, onError: e.onError, when: e.when }); }
function Hn(t, e) { w(t); let r = g.unwrapFieldPath(t), n = Gr(e); r.builder.addMetadataRule(te, () => n); }
function Gr(t) { return typeof t == "function" ? t : t === "blur" ? qr() : t > 0 ? $r(t) : zr; }
function $r(t) { return (e, r) => new Promise(n => { let i, s = () => { clearTimeout(i), n(); }; i = setTimeout(() => { r.removeEventListener("abort", s), n(); }, t), r.addEventListener("abort", s, { once: !0 }); }); }
function qr() { return (t, e) => new Promise(r => { e.addEventListener("abort", () => r(), { once: !0 }); }); }
function zr() { }
function Ot(t, e, r) { let n = wt({ source: t, computation: () => [], equal: v }), i = o => { let a = r(o); n.set(pt(a.error)), a.value !== void 0 && e(a.value), n.set(pt(a.error)); }, s = () => { n.set([]); }; return { errors: n.asReadonly(), setRawValue: i, reset: s }; }
function Gn(t, e) { let { parse: r, format: n } = e, i = Ot(t, t.set, r), s = wt(() => n(t())), o = s; o.parseErrors = i.errors; let a = o.set.bind(o), d = N(Ct, { self: !0, optional: !0 }); return d && (d.setParseErrors(i.errors), d.onReset = l => { i.reset(); let m = l !== void 0 ? l : t(); a(n(m)); }), o.set = l => { i.setRawValue(l), a(l); }, o.update = l => { o.set(l(s())); }, o; }
var ze = class {
    field;
    ...
    get errors() { return mt(this.field().errors()); }
    ...
    get status() { if (this.field().disabled())
        ...
        return "PENDING"; throw new Ve(1910, !1); }
    valueAccessor = null;
    hasValidator(e) { return e === At.required ? this.field().required() : !1; }
    updateValueAndValidity() { }
}, We = { disabled: "disabled", disabledReasons: "disabledReasons", dirty: "dirty", errors: "errors", hidden: "hidden", invalid: "invalid", max: "max", maxLength: "maxLength", min: "min", minLength: "minLength", name: "name", pattern: "pattern", pending: "pending", readonly: "readonly", required: "required", touched: "touched" }, Wr = (() => { let t = {}; for (let e of Object.keys(We))
    t[We[e]] = e; return t; })();
function Ze(t, e) { let r = Wr[e]; return t[r]?.(); }
var Qe = Object.values(We);
function ue() { return {}; }
function O(t, e, r) { return Object.is(t[e], r) ? !1 : (t[e] = r, !0); }
function Yr(t, e, r) { let n; if (Ft(t) && r.isBadInput(t))
    ...
    if (t.value === "")
        return { value: null };
    let i = Xr(t.value);
    return i === void 0 ? { error: new de } : { value: i };
} return { value: t.value }; }
function vt(t, e) { switch (t.type) {
    case "checkbox":
        t.checked = e;
        return;
    case "radio":
...
function Xr(t) { let e = Number(t); if (!(Number.isNaN(e) || !Object.is(e, parseFloat(t))))
    return e; }
function Nt(t, e) { isNaN(e) ? t.value = "" : t.valueAsNumber = e; }
function Ft(t) { return t.tagName === "INPUT"; }
...
function _t(t, e, r) { return e instanceof Date && (t === "min" || t === "max") && (r === "date" || r === "month") ? Qr(e, r) : e; }
function Jr(t, e) { t.listenToCustomControlModel(n => e.state().controlValue.set(n)), t.listenToCustomControlOutput("touch", () => e.state().markAsTouched()), e.registerAsBinding(t.customControl); let r = ue(); return () => { let n = e.state(), i = n.controlValue(); O(r, "controlValue", i) && t.setCustomControlModelInput(i); for (let s of Qe) {
    let o;
    if (s === "errors" ? o = e.errors() : o = Ze(n, s), O(r, s, o) && (t.setInputOnDirectives(s, o), e.elementAcceptsNativeProperty(s) && !t.customControlHasInput(s))) {
        let a = _t(s, o, e.nativeFormElement.type);
        Xe(e.renderer, e.nativeFormElement, s, a);
    }
} }; }
function en(t) { return typeof t == "object" && t !== null; }
function tn(t, e) { let r = ue(); e.controlValueAccessor.registerOnChange(i => { r.controlValue = i, e.state().controlValue.set(i); }), e.controlValueAccessor.registerOnTouched(() => e.state().markAsTouched()); let n = e.injector.get(wr, null, { optional: !0, self: !0 }); if (n) {
    let i;
    for (let d of n)
        en(d) && d.registerOnValidatorChange && (i ??= St(0), d.registerOnValidatorChange(() => { i.update(l => l + 1); }));
    let s = n.map(d => typeof d == "function" ? d : d.validate.bind(d)), o = At.compose(s), a = K(() => { i?.(); let d = o ? o(e.interopNgControl.control) : null; return gt(d, e.interopNgControl.control); });
    e.parseErrorsSource.set(a);
} return e.registerAsBinding({ reset: () => { let i = e.state().value(); r.controlValue = i, T(() => e.controlValueAccessor.writeValue(i)); } }), () => { let i = e.state(), s = i.controlValue(); O(r, "controlValue", s) && T(() => e.controlValueAccessor.writeValue(s)); for (let o of Qe) {
    let a = Ze(i, o);
    if (O(r, o, a)) {
        let d = t.setInputOnDirectives(o, a);
        o === "disabled" && e.controlValueAccessor.setDisabledState ? T(() => e.controlValueAccessor.setDisabledState(a)) : !d && e.elementAcceptsNativeProperty(o) && Xe(e.renderer, e.nativeFormElement, o, a);
    }
} }; }
...
function sn(t, e, r, n) { let i = !1, s = e.nativeFormElement, o = ue(), a = Ot(() => e.state().value(), d => { o.controlValue = d, e.state().controlValue.set(d); }, d => Yr(s, e.state().value, n)); return r.set(a.errors), e.onReset = () => { a.reset(); let d = e.state().value(); o.controlValue = d, vt(s, d); }, t.listenToDom("input", () => a.setRawValue(void 0)), t.listenToDom("blur", () => e.state().markAsTouched()), Ft(s) && Zr(s) && n.watchValidity(e.destroyRef, s, () => a.setRawValue(void 0)), e.registerAsBinding(), s.tagName === "SELECT" && rn(s, () => { i && (s.value = e.state().controlValue()); }, e.destroyRef), () => { let d = e.state(); for (let m of Qe) {
    let j = Ze(d, m);
    if (O(o, m, j) && (t.setInputOnDirectives(m, j), e.elementAcceptsNativeProperty(m))) {
        let le = _t(m, j, s.type);
        Xe(e.renderer, s, m, le);
    }
} let l = d.controlValue(); O(o, "controlValue", l) && vt(s, l), i = !0; }; }
var Vt = (() => { class t {
    ...
    return t;
})(), an = Symbol(), Mt = new Et(""), $n = (() => { class t {
    field = Rt.required({ alias: "formField" });
    state = K(() => this.field()());
    renderer = N(pr);
    destroyRef = N(br);
    injector = N(Tt);
    element = N(vr).nativeElement;
    elementIsNativeFormElement = Sr(this.element);
    elementAcceptsTextualValues = Rr(this.element);
    _elementAcceptsMinMax;
    nativeFormElement = this.elementIsNativeFormElement ? this.element : void 0;
    focuser = r => this.element.focus(r);
    controlValueAccessors = N(Tr, { optional: !0, self: !0 });
    config = N(It, { optional: !0 });
    validityMonitor = N(Vt);
    parseErrorsSource = St(void 0);
    _interopNgControl;
    get interopNgControl() { return this._interopNgControl ??= new ze(this.state); }
    ...
    get controlValueAccessor() { return !this.controlValueAccessors || this.controlValueAccessors.length === 0 ? this.interopNgControl?.valueAccessor ?? void 0 : Dr(this.interopNgControl, this.controlValueAccessors) ?? void 0; }
    installClassBindingEffect() { let r = Object.entries(this.config?.classes ?? {}).map(([i, s]) => [i, K(() => s(this))]); if (r.length === 0)
        return; let n = ue(); Nr({ write: () => { for (let [i, s] of r) {
            let o = s();
            O(n, i, o) && (o ? this.renderer.addClass(this.element, i) : this.renderer.removeClass(this.element, i));
    ...
    registerAsBinding(r) { if (this.isFieldBinding)
        throw new Ve(1913, !1); this.isFieldBinding = !0, this.installClassBindingEffect(), r?.focus && (this.focuser = n => r.focus(n)), r?.reset && (this.resetter = () => r.reset()), Dt(n => { let i = this.state(); i.nodeState.formFieldBindings.update(s => [...s, this]), n(() => { i.nodeState.formFieldBindings.update(s => s.filter(o => o !== this)); }); }, { injector: this.injector }); }
    [an];
    ...
    elementAcceptsNativeProperty(r) { if (!this.elementIsNativeFormElement)
        return !1; switch (r) {
        case "min":
        case "max": return this._elementAcceptsMinMax ??= Cr(this.element);
        case "minLength":
    ...
    static \u0275fac = function (n) { return new (n || t); };
    static \u0275dir = b.\u0275\u0275defineDirective({ type: t, selectors: [["", "formField", ""]], inputs: { field: [1, "formField", "field"] }, exportAs: ["formField"], features: [b.\u0275\u0275ProvidersFeature([{ provide: Mt, useExisting: t }, { provide: Ar, useFactory: () => N(t).interopNgControl }, { provide: Ct, useFactory: () => N(Mt, { self: !0 }) }]), b.\u0275\u0275ControlFeature("formField")] });
} return t; })(), qn = (() => { class t {
    fieldTree = Rt.required({ alias: "formRoot" });
    onSubmit(r) { r.preventDefault(), T(() => { let n = this.fieldTree(); n().structure.fieldManager.submitOptions && _e(n); }); }
    static \u0275fac = function (n) { return new (n || t); };
...
} return t; })(), dn = (t, e) => { let r = N(Tt); return new Promise((n, i) => { Dt(() => { T(() => { un(t, e, r).then(n, i); }); }); }); };
function un(t, e, r) {
    return I(this, null, function* () {
        ...
        yield Er({ name: e.name, description: e.description, inputSchema: i, execute: s => I(null, null, function* () {
                return n.value.set(s), (yield _e(t)) ? { content: [{ type: "text", text: "Form submitted successfully." }] } : { content: [{ type: "text", text: `Form submission failed:
${n.errorSummary().map(d => { let l = d.fieldTree().structure.pathKeys().join("."); return `${l ? `${l}: ` : ""}${d.message || d.kind}`; }).join(`
`)}` }] };
            }) }, r);
    });
}
function Ye(t) { let e = t.value(); if (typeof e == "string")
...
function zn() { return Mr([{ provide: Fe, useValue: dn }]); }
export { p as BaseNgValidationError, $e as EmailValidationError, Mt as FORM_FIELD, $n as FormField, qn as FormRoot, se as IS_ASYNC_VALIDATION_RESOURCE, ae as MAX, Ae as MAX_DATE, ke as MAX_LENGTH, Ie as MAX_NUMBER, oe as MIN, De as MIN_DATE, Pe as MIN_LENGTH, Ce as MIN_NUMBER, Be as MaxDateValidationError, He as MaxLengthValidationError, xe as MaxValidationError, G as MetadataKey, S as MetadataReducer, je as MinDateValidationError, Ue as MinLengthValidationError, Ke as MinValidationError, de as NativeInputParseError, Pn as NgValidationError, Oe as PATTERN, Ge as PatternValidationError, Te as REQUIRED, Le as RequiredValidationError, qe as StandardSchemaValidationError, ht as apply, dr as applyEach, ft as applyWhen, ur as applyWhenValue, Re as createLimitSelectionKey, ut as createManagedMetadataKey, f as createMetadataKey, Hn as debounce, Cn as disabled, kn as email, xr as emailError, ar as form, An as hidden, On as max, Fn as maxDate, Vr as maxDateError, _r as maxError, _n as maxLength, Kr as maxLengthError, h as metadata, Vn as min, Ln as minDate, Fr as minDateError, Or as minError, Kn as minLength, Lr as minLengthError, jn as pattern, jr as patternError, zn as provideExperimentalWebMcpForms, Dn as provideSignalFormsConfig, In as readonly, xn as required, kr as requiredError, lr as schema, Hr as standardSchemaError, _e as submit, Gn as transformedValue, D as validate, kt as validateAsync, Un as validateHttp, Bn as validateStandardSchema, Ur as validateTree, an as \u0275NgFieldDirective };
