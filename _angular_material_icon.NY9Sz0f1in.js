...
var V = ["*"], Y = new b("MAT_ICON_DEFAULT_OPTIONS"), J = new b("mat-icon-location", { providedIn: "root", factory: () => { let i = f(O), h = i ? i.location : null; return { getPathname: () => h ? h.pathname + h.search : "" }; } }), A = ["clip-path", "color-profile", "src", "cursor", "fill", "filter", "marker", "marker-start", "marker-mid", "marker-end", "mask", "stroke"], K = A.map(i => `[${i}]`).join(", "), G = /^url\(['"]?#(.*?)['"]?\)$/, pt = (() => {
    class i {
        _elementRef = f(H);
        _iconRegistry = f(R);
        _location = f(J);
...
export { Y as MAT_ICON_DEFAULT_OPTIONS, J as MAT_ICON_LOCATION, pt as MatIcon, vt as MatIconModule, R as MatIconRegistry, F as getMatIconFailedToSanitizeLiteralError, E as getMatIconFailedToSanitizeUrlError, C as getMatIconNameNotFoundError, M as getMatIconNoHttpProviderError };
